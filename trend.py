...
class TrendVis(object):
                ...
                if "wiki_id" not in res:
                    ...
                    for t in res["wiki"]:
                        ...
                        if " " in t and t not in self.stop_words:
                            #reg = r"(^.*\s?is$)|(is\s.*?$)"
                            reg = r"|".join(["(^.*\s%s$)|(^%s\s.*$)" % (x, x) for x in
                                             ["in", "is", "are", "the", "a", "been", "but", "was", "be", "a", "there",
                                              "this", "that", "to", "of", "not", "so", "we", "with", "than", "for",
                                              "and", "wa", "it", "almost", "an", "al"]])
    ...
    def caculate_term_frequence_given_time(self):
        ...
        self.term_freq_given_person_time = [np.zeros((self.num_terms, self.num_authors)) for i in
                                            range(self.num_time_slides)]
    ...
    def caculate_term_frequence_(self):
        ...
        for i in range(self.num_time_slides):
            for y in self.time_slides[i]:
                for d in self.document_list_given_time[y]:
                    text = (self.document_list[self.document_index[d]].names.lower() + " . " + self.document_list[
                        self.document_index[d]].abs.lower())
                    for t in range(self.num_terms):
                        if self.term_list[t] in text:
                            self.term_freq[t] += 1
                            self.term_freq_given_time[i, t] += 1
    ...
    def global_clustering(self):
        ...
        self.global_cluster_labels = [[None for i in range(self.num_local_clusters)] for j in
                                      range(self.num_time_slides)]
    ...
    def build_graph(self):
        ...
        for time in range(self.num_time_slides):
            ...
            for i, cluster in enumerate(self.global_clusters[time]):
                ...
                "key": [{"term": self.term_list[k], "w": int(self.term_freq_given_time[time][k])} for k in
                        sorted_terms_given_time],
        ...
        for t in sorted_terms:
            ...
            item = {"t": t, "idx": int(term_index),
                    ...
                    "first": [{"p": p, "y": self.term_first_given_person[term_index][p]} for p in
                              self.term_first_given_person[term_index]],
